...
static int check_order(RadixMap *map) 
{
	...
	// only signal errors if any (should not be any) 
...
static int test_search(RadixMap *map) {
	...
	for (i = map->end / 2; i < map->end; i++) {
		...
		check(found->data.key == d->data.key, "got wrong result: %p %u looking for %u at %u", found, found->data.key, d->data.key, i);
	}
...
static char *test_operations() {
	...
	while (map->end > 0) {
		...
		// test that th end is now the old value,
